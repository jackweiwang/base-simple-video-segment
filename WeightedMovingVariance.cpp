...
WeightedMovingVariance::WeightedMovingVariance() : firstTime(true), enableWeight(true), 
...
//unused
cv::Mat WeightedMovingVariance::computeWeightedMean(const std::vector<cv::Mat> &v_img_input_f, const std::vector<double> &weights)
{
  cv::Mat img;
  return img;
}
